export const aquaTherapyMumbaiFaqs = [
  {
    question: 'What is aqua therapy and how does it work?',
    answer:
      'Aqua therapy is physiotherapy performed on a temperature-controlled aqua treadmill. Water reduces the load on your joints through buoyancy - in chest-deep water, your body bears only 25% of its normal weight. Hydrostatic pressure from the water reduces swelling and improves circulation. Resistance from water movement strengthens muscles without the impact stress of land-based exercise. At R3BOOT, sessions are supervised by our physiotherapy team who guide your movements and adjust intensity based on your recovery stage.',
  },
  {
    question: 'Is aqua therapy the same as hydrotherapy?',
    answer:
      'The terms are used interchangeably. Hydrotherapy refers broadly to any therapeutic use of water. Aqua therapy (or aquatic physiotherapy) is the clinical subspecialty where a qualified physiotherapist supervises and prescribes exercises in water. At R3BOOT, your sessions are led by a physiotherapist - not a general instructor - which means the program is clinically structured for your specific injury, surgery, or condition.',
  },
  {
    question: 'Is aqua therapy safe after surgery?',
    answer:
      'Yes, aqua therapy is widely used for post-surgical rehabilitation once wound healing is confirmed and your surgeon or physiotherapist clears you for aqua treadmill sessions. The reduced weight-bearing in water allows movement and strength work that would be too painful or risky on land at the same stage of recovery. Our physiotherapists review your surgical history and post-op timeline before your first session.',
  },
  {
    question: 'What conditions does aqua therapy help with at R3BOOT?',
    answer:
      'Aqua therapy at R3BOOT is used for post-surgical rehabilitation (knee replacement, hip replacement, ACL repair), sports injuries, lower back pain, osteoarthritis, rheumatoid arthritis, shoulder injuries, ankle and foot injuries, and return-to-sport conditioning after injury. Our physiotherapists will assess whether aqua therapy is the right starting point or whether land-based physiotherapy should come first.',
  },
  {
    question: 'How is aqua therapy different from regular physiotherapy?',
    answer:
      'Land-based physiotherapy applies load under full gravity. Aqua therapy uses buoyancy to reduce that load, hydrostatic pressure to manage swelling, and water resistance to provide graduated strengthening - all simultaneously. This allows earlier, pain-free movement in the recovery cycle compared to land. The two are not competing: aqua therapy accelerates the early stages of rehabilitation, and land-based physiotherapy follows once adequate strength and range of motion are restored.',
  },
  {
    question: 'Do I need to know how to swim for aqua therapy?',
    answer:
      'No swimming ability is required. Aqua therapy sessions at R3BOOT are conducted on the aqua treadmill where you can stand comfortably. Exercises are performed upright, with your physiotherapist beside the aqua treadmill at all times. The goal is therapeutic movement, not swimming.',
  },
  {
    question: 'How long is a session and how many sessions will I need?',
    answer:
      'A standard aqua therapy session at R3BOOT is 45 to 60 minutes including your pre-session review and post-session check-in. The number of sessions depends on your condition: post-surgical patients typically require 8 to 12 sessions over 6 to 8 weeks, while sports injury rehabilitation may require 4 to 8 sessions. Your physiotherapist will give you a session estimate after your initial assessment.',
  },
  {
    question: 'What should I bring to an aqua therapy session at R3BOOT?',
    answer:
      'Bring swimwear suitable for exercise, a towel, and any recent medical reports or post-operative discharge summaries. If you have had surgery, bring your surgeon\'s notes and clearance. Arrive hydrated and avoid a heavy meal within 90 minutes of your session. We provide everything else.',
  },
  {
    question: 'Can aqua therapy help with knee pain?',
    answer:
      'Yes. Knee pain - whether from osteoarthritis, ligament injury, post-replacement, or sports overuse - is one of the most common conditions treated with aqua therapy. Water reduces the compressive load through the knee by up to 75%, allowing you to perform strength and mobility exercises that would be too painful on land. For knee osteoarthritis specifically, aqua therapy has strong evidence for pain reduction and functional improvement.',
  },
  {
    question: 'Is aqua therapy good for arthritis?',
    answer:
      'Aqua therapy is one of the most recommended interventions for both osteoarthritis and rheumatoid arthritis. Warm water relaxes muscles and reduces joint stiffness. Buoyancy removes the compressive load that makes land-based movement painful. Hydrostatic pressure reduces joint swelling. The resistance of water provides gentle strengthening without joint stress. Our physiotherapists design arthritis-specific protocols that respect your pain thresholds and adapt as your condition improves.',
  },
  {
    question: 'Can I combine aqua therapy with other services at R3BOOT?',
    answer:
      'Yes, and this is often how we see the best outcomes. Aqua therapy pairs well with land-based physiotherapy for progressive rehabilitation, sports massage for muscle recovery between sessions, and clinical pilates for core stability as you return to activity. Our physiotherapy team coordinates your program across services so there is no duplication or conflicting load.',
  },
  {
    question: 'How much does aqua therapy cost at R3BOOT Mumbai?',
    answer:
      'Please contact R3BOOT directly for current pricing and package options. Sessions can be booked individually or as part of a structured rehabilitation package. Call or WhatsApp us at +91 97023 68612 and our team will advise on the right program for your condition and budget.',
  },
  {
    question: 'Is aqua therapy good for back pain?',
    answer:
      'Aqua therapy is highly effective for lower back pain. In chest-deep water, spinal compression reduces significantly, allowing movement, core strengthening, and flexibility work that causes pain on land. Common aqua therapy exercises for back pain include water walking, trunk rotation, pelvic tilts, and buoyancy-supported stretching. At R3BOOT, your physiotherapist designs the program around your specific diagnosis - disc bulge, muscle strain, spondylosis, or post-surgical back - since the right exercises vary by condition.',
  },
  {
    question: 'What aqua therapy exercises help with neck and shoulder pain?',
    answer:
      'Aqua therapy exercises for neck and shoulder pain typically include supported arm circles, shoulder blade retractions against water resistance, rotator cuff strengthening in neutral positions, neck mobility work with buoyancy support, and scapular stability movements. Water reduces load through the shoulder joint, allowing range-of-motion and strengthening work earlier than is possible on land. At R3BOOT, shoulder and neck programs are prescribed by our physiotherapists based on your specific diagnosis - rotator cuff, labral injury, impingement, or post-surgical.',
  },
  {
    question: 'What aqua therapy exercises are used for back pain?',
    answer:
      'Common aqua therapy exercises for back pain include water walking (forward and backward), trunk rotation against water resistance, standing hip extensions, knee-to-chest stretches with buoyancy support, and bridging in shallow water. For disc-related pain, buoyancy-assisted lumbar traction movements are used. Your physiotherapist at R3BOOT selects exercises based on your diagnosis and pain response - not all back pain exercises are appropriate for all conditions.',
  },
  {
    question: 'What aqua therapy exercises help with knee pain?',
    answer:
      'Aqua therapy exercises for knee pain include straight leg raises, mini squats against water resistance, water walking at variable speeds on the aqua treadmill, terminal knee extensions, and leg swings in supported standing. The sequence depends on your knee condition - post-replacement, ACL repair, and osteoarthritis each require different progressions and load levels. At R3BOOT, your physiotherapist adjusts exercises and treadmill speed to control how much load goes through your knee at each stage of recovery.',
  },
  {
    question: 'Is aqua therapy effective for a herniated disc?',
    answer:
      'Yes, aqua therapy is effective for herniated disc rehabilitation. Water reduces spinal compression significantly, allowing movement, neural mobilisation, and core strengthening that would aggravate symptoms on land. Warm water also relaxes the paraspinal muscle spasm that often accompanies disc herniation. At R3BOOT, herniated disc programs focus on pain-free range of motion first, progressing to core stability and functional loading as symptoms settle. Your physiotherapist reviews your MRI or imaging before designing your program.',
  },
]
