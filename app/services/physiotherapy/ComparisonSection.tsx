...
import { Users, ClipboardList, Target, TrendingUp, Layers, Stethoscope, Building2, Check } from 'lucide-react'

const ROWS = [
  {
    icon: Target,
    label: 'Approach',
    standard: 'Session-by-session symptom relief',
    r3boot: 'Root cause, progressive programming',
  },
  {
    icon: Users,
    label: 'Team',
    standard: 'General physiotherapists',
    r3boot: 'Specialists with advanced certifications',
  },
  {
    icon: Layers,
    label: 'Integration',
    standard: 'Physio only',
    r3boot: 'Physio + massage + recovery modalities',
  },
  {
    icon: ClipboardList,
    label: 'Techniques',
    standard: 'Standard exercise and manual',
    r3boot: 'Manual therapy, dry needling, aquatic rehab',
  },
  {
    icon: Building2,
    label: 'Setting',
    standard: 'Standalone clinic',
    r3boot: 'Integrated recovery center',
  },
  {
    icon: TrendingUp,
    label: 'Goal',
    standard: 'Short-term pain relief',
    r3boot: 'Sustainable long-term outcomes',
  },
]
      ...
      <div className="container mx-auto px-5 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-2xl mx-auto mb-12 sm:mb-14">
          ...
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-black text-gray-900 dark:text-white tracking-tight leading-[1.1] mb-5">
            R3BOOT physiotherapy is more than{' '}
            <span className="text-[#7B5FCA]">a standard clinic visit.</span>
          </h2>
          <div className="w-10 h-[2px] bg-[#7B5FCA] mx-auto mb-5" />
          <p className="text-gray-500 dark:text-white/45 text-base leading-relaxed">
            Most clinics treat the flare. We assess why it started, build a progressive plan, and integrate recovery tools you cannot get in a standalone physio room.
          </p>
        </div>
              ...
              <div key={i} className="rounded-2xl border border-gray-100 dark:border-white/[0.08] bg-white dark:bg-[#0D0D13] overflow-hidden">
                ...
                <div className="px-5 py-4 border-b border-gray-100 dark:border-white/[0.06]">
                  <div className="flex items-center gap-2 mb-1.5">
                    <Building2 className="w-3.5 h-3.5 text-gray-400 dark:text-white/35" strokeWidth={2} />
                    <span className="text-gray-400 dark:text-white/35 text-[10px] font-black tracking-widest uppercase">Standard Physio Clinic</span>
                  </div>
                  <p className="text-gray-500 dark:text-white/45 text-[14px] leading-snug pl-[22px]">{row.standard}</p>
                </div>
                ...
                <div className="px-5 py-4 bg-[#513394]/[0.05] dark:bg-[rgba(123,95,202,0.08)]">
                  <div className="flex items-center justify-between mb-1.5">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[#513394] dark:text-[#A78BFA] text-[10px] font-black tracking-widest uppercase">R3BOOT Physiotherapy</span>
                    </div>
                    <div className="w-5 h-5 rounded-full bg-[#7B5FCA] flex items-center justify-center shrink-0">
                      <Check className="w-3 h-3 text-white" strokeWidth={3} />
                    </div>
                  </div>
                  <p className="text-[#513394] dark:text-[#C4B5FD] font-bold text-[14px] leading-snug pl-[22px]">{row.r3boot}</p>
                </div>
              </div>
            )
          })}
