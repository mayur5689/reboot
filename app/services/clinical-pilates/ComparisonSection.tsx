...
export type ComparisonRow = {
  ...
  label: string
  groupClass: string
  /** Right-column value (R3BOOT / physio-led). */
  right: string
}
...
const DEFAULT_ROWS: ComparisonRow[] = [
  {
    icon: Users,
    label: 'Who leads it',
    groupClass: 'Fitness instructor, group-trained',
    right: 'Registered physiotherapist',
  },
  {
    icon: ClipboardList,
    label: 'Starting point',
    groupClass: 'Fixed class sequence, same for everyone',
    right: 'Formal assessment of your diagnosis',
  },
  {
    icon: Target,
    label: 'Exercise selection',
    groupClass: 'General fitness moves',
    right: 'Prescribed to your specific deficit',
  },
  {
    icon: Activity,
    label: 'Progression',
    groupClass: 'Follows the class calendar',
    right: 'Based on objective re-assessment',
  },
  {
    icon: ShieldCheck,
    label: 'Post-surgical safety',
    groupClass: 'Not screened for your condition',
    right: 'Graduated loading within clinical limits',
  },
  {
    icon: TrendingUp,
    label: 'Best for',
    groupClass: 'General fitness maintenance',
    right: 'Back pain, post-surgery, scoliosis, sport rehab',
  },
]
...
export function ComparisonSection({
  title,
  subtitle,
  leftColumnLabel = 'Group Pilates Class',
  rightColumnLabel = 'Clinical Pilates',
  rightColumnMobileLabel,
  rows = DEFAULT_ROWS,
  ...
  const mobileRight = rightColumnMobileLabel ?? rightColumnLabel

  const resolvedTitle =
    title ?? (
      <>
        Clinical Pilates is more than <span className="text-[#7B5FCA]">a group class.</span>
      </>
    )
    ...
    <section className="py-10 md:py-16 lg:py-28 bg-[#F8F9FA] dark:bg-[#09090E]">
      <div className="container mx-auto px-5 sm:px-6 lg:px-8">
        <div className="hidden md:block text-center max-w-2xl mx-auto mb-12 sm:mb-14">
          ...
          <p className="text-gray-500 dark:text-white/45 text-base leading-relaxed">{resolvedSubtitle}</p>
        </div>
          ...
          <div className="grid grid-cols-2">
            ...
            {rows.map((row, i) => {
              ...
              return (
                <Fragment key={i}>
                  <div
                    className={`flex items-center justify-center px-2.5 py-3.5 text-center ${!isLast ? 'border-b border-gray-100 dark:border-white/[0.07]' : ''}`}
                  >
                    <span className="text-gray-500 dark:text-white/45 text-[11px] leading-snug">{row.groupClass}</span>
                  </div>
                  <div
                    className={`flex items-center justify-between gap-1.5 px-2.5 py-3.5 bg-[rgba(123,95,202,0.06)] ${!isLast ? 'border-b border-[#7B5FCA]/20' : ''}`}
                  >
                    <span className="text-[#513394] dark:text-[#C4B5FD] font-bold text-[11px] leading-snug">{row.right}</span>
          ...
          <div className="grid" style={{ gridTemplateColumns: COLS }}>
            ...
            >
              <div className="w-10 h-10 sm:w-11 sm:h-11 rounded-full border-[1.5px] border-gray-300 dark:border-white/30 bg-gray-100 dark:bg-white/[0.06] flex items-center justify-center shrink-0">
                <Users className="w-[18px] h-[18px] text-gray-500 dark:text-white/60" strokeWidth={2} />
              </div>
            ...
            {rows.map((row, i) => {
              ...
              return (
                <Fragment key={i}>
                  ...
                  >
                    <span className="text-gray-500 dark:text-white/45 text-[14px] sm:text-[16px] leading-snug">{row.groupClass}</span>
                  </div>
