...
import { Users, ClipboardList, Target, TrendingUp, ShieldCheck, Activity, Stethoscope, Check } from 'lucide-react'
...
const ROWS = [
  {
    ...
    clinical: 'Registered physiotherapist',
  },
  {
    icon: ClipboardList,
    label: 'Starting point',
    groupClass: 'Fixed class sequence, same for everyone',
    clinical: 'Formal assessment of your diagnosis',
  },
  {
    icon: Target,
    label: 'Exercise selection',
    groupClass: 'General fitness moves',
    clinical: 'Prescribed to your specific deficit',
  },
  {
    icon: Activity,
    label: 'Progression',
    groupClass: 'Follows the class calendar',
    clinical: 'Based on objective re-assessment',
  },
  {
    icon: ShieldCheck,
    label: 'Post-surgical safety',
    groupClass: 'Not screened for your condition',
    clinical: 'Graduated loading within clinical limits',
  },
  {
    icon: TrendingUp,
    label: 'Best for',
    groupClass: 'General fitness maintenance',
    clinical: 'Back pain, post-surgery, scoliosis, sport rehab',
  },
]
      ...
      <div className="container mx-auto px-5 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-2xl mx-auto mb-12 sm:mb-14">
          ...
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-black text-white tracking-tight leading-[1.1] mb-5">
            Clinical Pilates is more than{' '}
            <span className="text-[#7B5FCA]">a group class.</span>
          </h2>
              ...
              <div key={i} className="rounded-2xl border border-white/[0.08] bg-[#0D0D13] overflow-hidden">
                ...
                <div className="px-5 py-4 border-b border-white/[0.06]">
                  <div className="flex items-center gap-2 mb-1.5">
                    <Users className="w-3.5 h-3.5 text-white/35" strokeWidth={2} />
                    <span className="text-white/35 text-[10px] font-black tracking-widest uppercase">Group Pilates Class</span>
                  </div>
                  <p className="text-white/45 text-[14px] leading-snug pl-[22px]">{row.groupClass}</p>
                </div>
                ...
                <div className="px-5 py-4" style={{ background: 'rgba(123,95,202,0.08)' }}>
                  <div className="flex items-center justify-between mb-1.5">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[#A78BFA] text-[10px] font-black tracking-widest uppercase">Clinical Pilates</span>
                    </div>
                    <div className="w-5 h-5 rounded-full bg-[#7B5FCA] flex items-center justify-center shrink-0">
                      <Check className="w-3 h-3 text-white" strokeWidth={3} />
                    </div>
                  </div>
                  <p className="text-[#C4B5FD] font-bold text-[14px] leading-snug pl-[22px]">{row.clinical}</p>
                </div>
              </div>
            )
          })}
        ...
        <div className="hidden md:block relative max-w-6xl mx-auto rounded-3xl border border-white/[0.08] bg-[#0D0D13] overflow-hidden">
          ...
          <div className="grid" style={{ gridTemplateColumns: COLS }}>
            ...
            {/* Highlight card for the Clinical Pilates column — spans every row */}
            <div
            ...
            <div style={{ gridColumn: 2, gridRow: HEADER_ROW }} className="flex items-center justify-center gap-3 h-full px-3 sm:px-7 py-6 sm:py-7">
              <div className="w-10 h-10 sm:w-11 sm:h-11 rounded-full border-[1.5px] border-white/30 bg-white/[0.06] flex items-center justify-center shrink-0">
                <Users className="w-[18px] h-[18px] text-white/60" strokeWidth={2} />
              </div>
