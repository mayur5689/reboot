  ...
  return (
    <div className="flex items-center gap-1.5 bg-gray-100 dark:bg-[#222] rounded-full px-3 py-1.5">
      {[...Array(5)].map((_, j) => (
        <svg
          key={j}
          className={`w-3.5 h-3.5 ${j < score ? 'fill-yellow-400' : 'fill-gray-300 dark:fill-white/20'}`}
          viewBox="0 0 20 20"
        >
          <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
        </svg>
      ))}
      <span className="text-gray-900 dark:text-white font-bold text-[13px] ml-0.5">{score.toFixed(1)}</span>
    </div>
  )
}
...
function TestimonialCard({ testimonial }: { testimonial: Testimonial }) {
  return (
    <figure className="w-[360px] lg:w-[400px] flex-shrink-0 bg-white dark:bg-[#161616] border border-gray-100 dark:border-white/[0.07] shadow-sm dark:shadow-none rounded-3xl p-6 lg:p-7 flex flex-col min-h-[380px] mr-5 lg:mr-6">
      ...
      <div className="h-px bg-gray-100 dark:bg-white/[0.07] mb-5" />
      ...
      <div className="flex items-center gap-3 mb-3">
        ...
        <div>
          <p className="text-gray-900 dark:text-white font-bold text-[15px] leading-tight">{testimonial.name}</p>
          <p className="text-gray-400 dark:text-white/40 text-[12px]">{testimonial.meta}</p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-1.5 mb-5">
        ...
        <span className="text-gray-400 dark:text-white/35 text-[12px]">{testimonial.ago}</span>
      </div>

      <blockquote className="text-gray-600 dark:text-white/80 text-[15px] leading-relaxed italic flex-grow mb-5 line-clamp-5">
